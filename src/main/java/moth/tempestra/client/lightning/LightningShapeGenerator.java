...
public final class LightningShapeGenerator {
    ...
    public static LightningBoltVisualData generate(LightningBolt entity, ClientLevel world) {
        Vec3 contact = entity.position();
        long worldTime = world == null ? 0L : world.getGameTime();
    ...
    private static ShapeBuild buildPaths(
        ...
        List<Vec3> impactContacts = new ArrayList<>(2);
        List<Vec3> sparkPoints = new ArrayList<>(mainPoints.size() + 8);
    ...
    private static GroundSplit buildConnectingGroundSplit(
        ...
        Vec3 secondaryContact = TempestraLightningSplits.chooseSecondaryGroundContact(primaryContact, origin, parentAxis, random, world);
        ...
        List<Vec3> points = buildMainPoints(start, secondaryContact, segmentCount, random);
        List<LightningSegment> segments = new ArrayList<>(segmentCount);
    ...
    private static int connectingGroundSplitSegmentCount(
        ...
        return Mth.clamp(segments, 3, maxSegments);
    }
    ...
    private static void maybeAddSplitBranches(
        ...
        Vec3 splitOrigin = splitSegments.get(0).start();
    ...
    private static List<LightningImpactRay> buildImpactRays(List<Vec3> contacts, Random random, boolean fullBolt, float intensity) {
    ...
    private static List<LightningSpark> buildSparks(List<Vec3> impactContacts, List<Vec3> sparkPoints, Random random, boolean fullBolt, float intensity) {
        ...
        for (Vec3 contact : impactContacts) {
        ...
        for (int index = 0; index < atmosphericCount; index++) {
            ...
            Vec3 base = sparkPoints.get(pointIndex);
            double scatterAngle = random.nextDouble(Angles.TAU);
            double scatter = random.nextDouble(0.35D, 2.2D);
            Vec3 position = base.add(Math.cos(scatterAngle) * scatter, random.nextDouble(-0.8D, 0.8D), Math.sin(scatterAngle) * scatter);
            Vec3 velocity = new Vec3(
    ...
    private static Vec3 chooseCompactOrigin(Vec3 contact, Random random) {
    ...
    private static double cloudBaseY(ClientLevel world, double contactY) {
        if (world != null) {
            float cloudsHeight = world.effects().getCloudHeight();
    ...
    private record GroundSplit(LightningPath path, List<Vec3> points, Vec3 contact) {
    }

    private record ShapeBuild(List<LightningPath> paths, List<Vec3> impactContacts, List<Vec3> sparkPoints) {
    }
}
